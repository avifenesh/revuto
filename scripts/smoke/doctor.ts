...
const fallback: ModelSpec = { ...m, model: 'fake-fallback' };
const config: ReviewerConfig = {
  vaultPath: '/tmp/reviewer-doctor',
  github: { tokenEnv: 'REVIEWER_NO_SUCH_TOKEN_ENV' },
  models: { review: { ...responses, fallbacks: [fallback] }, curator: m, distill: m, embedder: m },
...
assert.ok(report.models.some((p) => p.kind === 'chat' && p.roles.includes('review.fallback1') && p.model === 'fake-fallback'), 'has an explicit fallback chat probe');
...
console.log('PASS: doctor probes Responses + chat fallback + embedding + store backend (dedup OK)');
